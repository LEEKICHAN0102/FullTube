...
export const likeVideo=async(req,res)=>{
  ...
  const existLikeCounting=await userModel.findOne({likeVideo:id});

  let cnt;
  if(existLikeCounting){
    userModel.likeVideo.remove(id);
    await userModel.save();
    
    videoModel.meta.rating.remove(_id);
    await videoModel.save();
    cnt=videoModel.meta.rating.length;
    console.log(cnt);
    return res.status(201).json({cnt});
  }
  userModel.likeVideo.push(id);
  await userModel.save();
  videoModel.meta.rating.push(_id);
  await videoModel.save();

  cnt=video.meta.rating.length;
  console.log(cnt);
